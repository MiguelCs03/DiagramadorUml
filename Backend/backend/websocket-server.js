...
class CollaborationServer {
  constructor(port = 8081) {
    ...
    // Paths para persistencia de datos (auto-guardado)
  ...
  ensureDataDir() {
    try {
      if (!fs.existsSync(this.dataDir)) {
        ...
        console.log(`📁 [Persistencia] Carpeta creada: ${this.dataDir}`);
      } else {
        console.log(`📁 [Persistencia] Carpeta existente: ${this.dataDir}`);
      }
    } catch (err) {
      console.error('❌ [Persistencia] Error creando carpeta de datos:', err);
    }
  }

  getRoomFilePath(roomCode) {
    return path.join(this.dataDir, `${roomCode.toUpperCase()}.json`);
  }

  async saveRoomToDisk(room) {
    const filePath = this.getRoomFilePath(room.code);
    const payload = {
      id: room.id,
      code: room.code,
    ...
    try {
      ...
      console.log(`💾 [Persistencia] Diagrama guardado en ${filePath}`);
    } catch (err) {
      console.error('❌ [Persistencia] Error guardando sala a disco:', err);
    }
  }

  async loadRoomFromDisk(roomCode) {
    const filePath = this.getRoomFilePath(roomCode);
    try {
      ...
      const room = {
        id: data.id || uuidv4(),
        code: (data.code || roomCode).toUpperCase(),
        clients: new Set(),
        diagram: data.diagram || this.getDefaultDiagram(),
        createdAt: data.createdAt ? new Date(data.createdAt) : new Date(),
      };
      console.log(`📂 [Persistencia] Sala cargada desde disco: ${room.code}`);
      return room;
    } catch (err) {
      console.error('❌ [Persistencia] Error cargando sala desde disco:', err);
      return null;
    }
  }
  ...
  setupWebSocketHandlers() {
    this.wss.on('connection', (ws, req) => {
      ...
      ws.on('message', (message) => {
        console.log(`📨 [${clientIP}] Mensaje recibido:`, message);
        try {
          const data = JSON.parse(message);
          this.handleMessage(ws, data);
        } catch (error) {
          console.error(`❌ [${clientIP}] Error parsing message:`, error);
          this.sendError(ws, 'Invalid message format');
        }
      });

      ws.on('close', (code, reason) => {
        console.log(`👤 Client disconnected from ${clientIP} (code: ${code}, reason: ${reason})`);
  ...
  rooms.set(roomCode, room);
    ...
    // Guardar inmediatamente el estado inicial
    this.saveRoomToDisk(room);
  }

  async joinRoom(ws, { roomCode }) {
    let room = rooms.get(roomCode.toUpperCase());
    
    if (!room) {
      // Intentar cargar desde disco (p. ej., después de reinicio del servidor)
      room = await this.loadRoomFromDisk(roomCode.toUpperCase());
      if (room) {
  ...
  leaveRoom(ws, { roomCode }) {
    ...
    if (room.clients.size === 0) {
      // Quitar de memoria pero mantener archivo en disco (historial)
      rooms.delete(roomCode.toUpperCase());
      console.log(`�️ Room vacía removida de memoria (persistida en disco): ${roomCode.toUpperCase()}`);
    } else {
  ...
  updateDiagram(ws, { roomCode, diagram }) {
    ...
    // Auto-guardar a disco en cada cambio
    this.saveRoomToDisk(room);
